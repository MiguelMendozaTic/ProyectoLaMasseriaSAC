...
@Service
@Transactional
@SuppressWarnings("null") // Ignora todos los warnings de null safety en esta clase
public class CategoriaService {
    ...
    public boolean existeCategoriaPorNombre(String nombre) {
        return nombre != null && !nombre.trim().isEmpty() 
            ? categoriaRepository.findByNombre(nombre.trim().toUpperCase()).isPresent() 
            : false;
    }
    ...
    private void verificarNombreUnico(Categoria categoria) {
        Optional<Categoria> existente = categoriaRepository.findByNombre(
            categoria.getNombre().trim().toUpperCase()
        );
    ...
    private void actualizarCampos(Categoria destino, Categoria origen) {
        if (origen.getNombre() != null && !origen.getNombre().trim().isEmpty()) {
            String nuevoNombre = origen.getNombre().trim().toUpperCase();
